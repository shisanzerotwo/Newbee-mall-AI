...
public class NewBeeMallCategoryServiceImpl implements NewBeeMallCategoryService {
    ...
    public String saveCategory(GoodsCategory goodsCategory) {
        ...
        if (goodsCategoryMapper.insertSelective(goodsCategory) > 0) {
            // 后台修改分类后,删除首页分类树缓存
            redisCacheUtil.delete(Constants.INDEX_CATEGORY_CACHE_KEY);
    ...
    public Boolean deleteBatch(Integer[] ids) {
        ...
        //删除分类数据
        if (goodsCategoryMapper.deleteBatch(ids) > 0) {
            // 后台删除分类后,删除首页分类树缓存
            redisCacheUtil.delete(Constants.INDEX_CATEGORY_CACHE_KEY);
            return true;
        }
    ...
    public List<NewBeeMallIndexCategoryVO> getCategoriesForIndex() {
        // 先从 Redis 缓存读取分类树(完整三级树,Jackson 可直接序列化/反序列化)
        List<NewBeeMallIndexCategoryVO> cacheList = redisCacheUtil.get(
                Constants.INDEX_CATEGORY_CACHE_KEY,
                new TypeReference<List<NewBeeMallIndexCategoryVO>>() {
                });
        if (!CollectionUtils.isEmpty(cacheList)) {
            return cacheList;
        }
        // 缓存未命中:查库组装三级分类树,并写入缓存
        List<NewBeeMallIndexCategoryVO> categoryList = queryCategoriesForIndex();
        if (!CollectionUtils.isEmpty(categoryList)) {
            redisCacheUtil.set(Constants.INDEX_CATEGORY_CACHE_KEY, categoryList, Constants.INDEX_CACHE_TTL);
        }
        return categoryList;
    }

    /**
     * 原有的查库+组装三级分类树的逻辑(被缓存方法调用)
     */
    private List<NewBeeMallIndexCategoryVO> queryCategoriesForIndex() {
