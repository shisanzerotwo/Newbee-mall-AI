 ...
 *         表示二级分类节点，包含子分类列表（三级分类）和父分类ID
 */
public class SecondLevelCategoryVO implements Serializable {

    private Long categoryId;            // 二级分类ID
    private Long parentId;              // 父分类ID（对应一级分类ID）
    private Byte categoryLevel;         // 分类级别（2=二级分类）
    private String categoryName;        // 分类名称
    private List<ThirdLevelCategoryVO> thirdLevelCategoryVOS; // 三级分类列表
