...
### 7.1 第四轮：意图门禁 + prompt 硬化 → **10/10（首次全通过）**

第三轮的 #8 暴露了「该查不查」的**第三种形态**（3 次复跑 1 过 2 挂）：
模型用通用知识答「化妆品一般分为护肤/彩妆/香水…」、**完全不调工具**，而回答里没有价格/库存数字，
原有的「内容侧」门禁抓不到。据此扩展门禁并硬化 prompt（任务卡 `docs/TASK_GUARD2.md`）：

| 改动 | 内容 |
|---|---|
| 门禁扩展 | 从「回答含价格/库存特征」扩为「**意图 + 内容**双判定」：本轮无工具调用时，问题命中商城意图词（价格/库存/分类/订单/推荐…）也判违规 → 重问一次（仍违规则放行 + ERROR 留痕） |
| prompt 修正 | 原第 4 条「不确定的信息说”我帮您核实一下“」正是“只承诺不执行”的诱因 → 改为「先调用工具核实」 |
| prompt 新增 | ① 禁止用通用知识回答商城问题（分类必须查 `searchByCategory`）；② 禁止“只说不做” |

**第四轮验收证据**（编排者独立复跑）：
- `CsAgentServiceTest` **23/23**（含 5 个新用例：阳性 + 退换货阴性 + 天气阴性 + 有工具阴性 + 词表逐题断言）
- 全量 `mvn test` **177/177**；冒烟 **16/16**
- 真实 **#8 复跑 3/3 通过**（修复前 1/3）
- **完整 10 问 10/10** —— 历史：1/10 → 7/10 → 9/10 → 9/10 → **10/10**

**两点如实说明（含一次补实验 + 一次自我纠错）**：
1. **门禁的触发情况（已实测）**：
   - 硬化开启时的 5 轮真实问答里，门禁 **0 次触发**（模型被 prompt 引导主动调了工具）——
     也就是说这几轮 **10/10 是 prompt 硬化的功劳，不能记给门禁**
   - **补做的兜底实验**（临时把 prompt 还原成旧措辞、跑曾失败的 #8）：门禁 **触发 1 次**，
     且重问后的回答引用了真实商城数据（MUJI 基础润肤化妆水 22~127 元、400ml 已下架等）
     → **证明傅底在真实链路确实会拦截并救回**；实验后已完整还原（md5 比对一致）
   - ⚠️ 但 IT 仍判该题失败：它记录的是**首轮**的 `toolCalls`（空），看不到重问过程（注释里写明是故意的）
     → **门禁的救援效果无法被现有 IT 观测**，这是已知的设计张力
2. ⚠️ **自我纠错（2026-09-19）**：我最初写的是“门禁在真实链路 0 次触发”，其中“从未被触发过”
   是**错的** —— 根因是**中文 grep 打在 GBK 日志上**（UTF-8 模式 vs GBK 内容，永远 0 命中），
   属**假阴性**；详见 `踩坑记录` 第 27 条。同一根因还污染了另一条结论：
   提交 `e3b4f81` 的信息里写“13 次真实问答里工具循环上限 0 次触发”作回退 ZCode 第 7 条的依据 ——
   **实际是 2 次**。回退决定本身仍成立（依据换成：该条超出任务范围 + 有副作用实证 ——
   使回答变简短导致 #2 断言挂，回退后 #2 恢复、10/10），但**“无证据”这个理由是我查错了**。
3. 本轮曾出现 #2 失败（答“库存很充足”却不引用数字 → 数字一致性断言失败；**非幻觉**，工具调用正确）。
   定位到是执行者**额外加的一条 prompt**（“查到即总结”）使回答变简短所致 ——
   该条**超出任务范围且副作用已实证，已回退**；回退后 #2 恢复，10/10
