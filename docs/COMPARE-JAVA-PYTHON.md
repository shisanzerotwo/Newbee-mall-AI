...
（构建产物，不入 git；`java-results.tsv` 仍是第一轮全量 10 题结果，本轮抽样未覆盖它）。

**结论（如实）**：真正要验证的**数字一致性断言仍然没有被执行到** ——
#1/#2 都在模型给出工具调用之前就超时（工具轨迹为空，**不能**据此推断“模型不会调工具”）。
本轮新增的确定结论只有一条：#10 可复现通过，且“无关问题不调工具”与第一轮一致。

上游超时是**持续性现象**（两轮相隔数小时，同一批题仍 2/3 超时），
所以 §2b 的缺口在本轮**没有缩小**；补齐它需要稳定的模型通道，不是重跑次数的问题。

## 6. 结论

本轮 Java 回归集已经固定了 Python 的 10 个原始问题、对应工具预期和可复算数字规则，
但真实模型通道两轮都大面积超时，Python 旧凭证也已失效，所以**不能据本轮结果宣称答案质量
