...
**两点如实说明（不能忽略）**：
1. **门禁在真实问答中 0 次触发** —— #8 的修复实际来自 **prompt 硬化**（软约束让模型主动调了工具）；
   门禁是**未被用到的结构性傅底**，其正确性由单测（23/23）覆盖，但**尚未在真实链路被触发验证过**
2. 本轮曾出现 #2 失败（答“库存很充足”却不引用数字 → 数字一致性断言失败；**非幻觉**，工具调用正确）。
   定位到是执行者**额外加的一条 prompt**（“查到即总结”）使回答变简短所致 ——
   该条**无证据支撑且超出任务范围，已回退**；回退后 #2 恢复，10/10
