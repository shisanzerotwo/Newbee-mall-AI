...
- **我要 grep 的文本是中文吗？现在这个命令能正确输出中文吗？**（见 §6.5）

### 6.5 ⚠️ 复核 RAG 语料：`redis-cli` **别加 `--no-raw`**，且必须做阳性对照

**教训（2026-09-19 实测）**：用 `redis-cli --no-raw` 导出语料后 grep 中文，
**永远 0 命中** —— 因为在**没有任何输出**的情况下看起来就跟"红线通过"一模一样，
属于典型的**空跑伪装成结论**。空跑会让你得出"语料里没有价格字段"这种**想要的结果**，
比报错更危险。

实测对照（命令与输出都是实跑的）：

```bash
# ✅ 正确：不传 --no-raw。管道输出时 redis-cli 默认就是 raw 模式
docker exec newbee-mall-redis redis-cli FT.SEARCH goods_kb '*' RETURN 1 text LIMIT 0 1000 > /d/tmp/corpus.txt

# ❌ 错误：--no-raw 把中文逐字节转义成 \xNN，grep 中文必然空跑
docker exec newbee-mall-redis redis-cli --no-raw FT.SEARCH goods_kb '*' RETURN 1 text LIMIT 0 1000 > /d/tmp/corpus-bad.txt
```

| 命令 | 阳性对照 `商品名称` | 红线项 `价格：` | 文件里的中文长什么样 |
|---|---:|---:|---|
| 默认（raw） | **575** ✅ | 0 | `2) "商品名称：【联想..."`
| `--no-raw` | **0** ❌ | 0（**假 0**） | `2) "\xe5\x95\x86\xe5\x93\x81\xe5\x90\x8d..."` |

**规矩**：任何"扫语料统计关键词"的检查，**先跑阳性对照** ——
`grep -c 商品名称 corpus.txt` 应等于商品数（当前 **575**）。对照不过，结论一律作废。

**顺带记下正确的红线口径**（别把这两个数混为一谈）：

| 检查项 | 实测 | 判据 |
|---|---:|---|
| 结构化字段 `价格：` / `库存：` | **0 / 0** | ✅ **这才是红线的判据** |
| 裸词「价格」 | 2 | 欧莱雅品牌文案（"以合理的价格…"），非价格数据 |
| 裸词「元」 | 15 | 商品名/详情（"荣耀 8X 千**元**屏霸"、"三**单元**发声"），非泄漏 |

（`FT.INFO goods_kb` 同时确认：`dim=512`、`num_docs=588`、`hash_indexing_failures=0`）
