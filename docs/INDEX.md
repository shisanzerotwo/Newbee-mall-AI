...
>
> 维护约定：里程碑或数字变化时更新本文件；**任何数字必须带口径 + 快照点（§9）**（这是本项目踩过最多的坑）。
> 最后核实：**2026-09-24**（HEAD `1bf92c5`）
...
| 指标 | 值 | 口径 / 日期 |
|---|---|---|
| 提交 | **63**（快照） | `git log --oneline \| wc -l`，@`1bf92c5`（09-17 ~ 09-19 三天完成，09-24 补文档整理） |
...
| **提交数** | `git log --oneline \| wc -l` | 拿文档里几天前的数字；**忽略“写文档本身也会改变提交数”**（本文件首次提交就把 62 变成 63） |
...
| `D:/ObsidianChanku/ai学习/research/AI Agent学习/项目文档盘点与记忆同步 SOP.md` | ⭐ **本次整理流程的 7 步复盘 + 检查表**（可复用于任何项目；2026-09-24 新建） |
| `D:/ObsidianChanku/ai学习/research/AI Agent学习/会话总结 2026-09-24.md` | 本次会话复盘（含“数字刚写完就过期”的实例） |

---

## 9. 本文件怎么维护（约定 —— 别让索引自己烂掉）

1. **数字必须带口径 + 快照点**，并给出取当前值的命令。
   写法：`63 提交（2026-09-24 快照 @1bf92c5）`；**不要写“当前是 N”** ——
   写本文档这件事本身就会改变提交数（本文件首次提交就把 62 变成 63，当场验证过）。
2. **新文档必须挂进 §3 的某一类**，并在 §2 判断是否要加进某条角色路径。不挂 = 下一个人找不到它。
3. **历史任务卡（§4）不删也不改**：它们是决策留痕与派活模板，只在 §4 登记，**不当 backlog 读**。
4. **里程碑变化时三处同改**：本文件 §1 表格 + `STATUS.md` §3/§4 + `README.md` 状态表 ——
   避免再出现“三处三个数”（54/62 那次就是这么来的）。
5. **改过期结论时**：优先改成新结论；若那段是**当轮留痕**，保留原文 + 加一段指向新结论的注
   （例：`STATUS.md` §11 的“数字一致性断言仍未执行到”已加注指向 §8 第 9 条的 10/10），**不要直接删**。
6. 本文件放在仓库内（而非知识库），因为**要改代码的人不会去翻个人知识库**；
   方法论与面试话术则留在知识库（§8），两边分工不重复。
