...
9. ~~10 问回归的「数字一致性」断言~~ ✅ **已执行到且 10/10 全通过**（2026-09-19 第四轮：意图门禁 + prompt 硬化；历史 1/10 → 7/10 → 9/10 → **10/10**）；细节见 `COMPARE-JAVA-PYTHON.md` §7.1（含两点如实说明：门禁在真实链路 0 次触发；执行者多加的 prompt 已回退）
10. ~~生产化四项~~ ✅ **已完成**（2026-09-19，`docs/TASK_PROD.md` 交 claude 执行、codex 复核）：
    - **中文检索**：发现「**40% 是灌水指标**」——20 条里 **11 条题目本身无解**（商品库没那个类目）
      ...
      原始 20 条一条未删、两个指标同时输出（见 `RAG-EVAL.md` §2.3）
    - 扩候选集 / 调 RRF 权重：实测**无正收益**（`keyword-weight=2.0` 反而 8/9→7/9）→ **默认值不动**，
      参数做成 `@Value` 旋钮（默认 = 改动前行为），见 `RAG-EVAL.md` §2.4
    - `/api/cs/health` 收敛（抽成 `CsHealthController`，主控制器 −27 行）
    - XSS 行为验证**已接进 CI**（独立 job `xss-browser-it`；无 Chrome 时 skip 并用 `::warning::` 喊话，不静默空跑）
    - `CS_ENABLE_REAL_MODEL_IT` 守卫放宽为 `1 / true / yes`（原先只认字面 `1`，写 `=true` 会静默跳过）
11. **仍需产品/架构决策**（未开工）：限流改共享存储（多实例前提）→ 客服接口鉴权 → R12 延迟预算
    （免费额度下单次问答实测 24~80s，与 §2#14 承诺的「<8s」差一个数量级）
