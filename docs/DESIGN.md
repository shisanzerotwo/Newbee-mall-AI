...
- [ ] 首字延迟 < 1.5s、完整回答 < 8s、并发 10 不阻塞
      <br>⚠️ **实测反例（R12）**：agnes 免费额度受限时单次问答 **24~80s**。该指标需在**付费/稳定额度**下考核，或按 R12 的结论重新取值，不要直接拿它当验收依据。
...
| **R12** | **免费额度下的失败/延迟预算失控**：agnes 免费额度受限时，单次问答**实测 24~80s**（`15:30:58→15:32:18` 为 80s 后失败；`15:35:16→15:35:40` 为 24s 后成功），与 §2#14 承诺的「完整回答 **<8s**」**差一个数量级**；而 `max-model-retries` × `timeout-seconds` 的最坏预算还会吃掉 SSE 单连接 120s 上限的大半 | 高 | 中 | M2-5 已收紧预算：`cs.model.timeout-seconds` 60→**15**、`cs.agent.max-model-retries` 3→**2**（最坏 ≈ 15+4+15 = **34s**），失败时给可读话术（「当前咨询较多，请稍后再试」）；**仍未解决**的是「免费额度本身的延迟下限」—— 需在 M3 评估付费/更快的模型通道，或把 §2#14 的 `>8s` 指标明确限定为「付费额度下」 | 免费额度受限时首字延迟 > 5s，或单次问答 > 15s（实测已多次触达） |
